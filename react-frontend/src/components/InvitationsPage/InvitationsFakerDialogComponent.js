...
import invitationsFakerFactory from "./invitationsFakerFactory";
...
const InvitationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = invitationsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("invitations").create(elem));
...
export default connect(mapState, mapDispatch)(InvitationsFakerDialogComponent);
