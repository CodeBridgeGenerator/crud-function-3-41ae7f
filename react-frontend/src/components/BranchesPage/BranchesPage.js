...
const BranchesPage = (props) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("branches").remove(data[selectedEntityIndex]?._id);
            ...
            console.log({ error });
            props.alert({ title: "Branches", type: "error", message: error.message || "Failed delete record" });
        }
    };
        ...
        <div className="mt-5"  style={{minHeight : 'calc(90vh - 9rem)'}}>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="branches-datatable">
                    <BranchesDatatable items={data} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} />
                 </div>
            </div>
            <AreYouSureDialog header="Delete" body="Are you sure you want to delete this record?" show={showAreYouSureDialog} onHide={() => setShowAreYouSureDialog(false)} onYes={() => deleteRow()} />
            <BranchesEditDialogComponent entity={data[selectedEntityIndex]} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <BranchesCreateDialogComponent show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
