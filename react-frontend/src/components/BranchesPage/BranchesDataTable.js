...
const BranchesDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
     
    const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.branch}</p>
    const checkboxTemplate2 = (rowData, { rowIndex }) => <Checkbox checked={rowData.isDefault}  ></Checkbox>
    ...
    return (
        ...
        currentPageReportTemplate="{first} to {last} of {totalRecords}" paginatorLeft={paginatorLeft} paginatorRight={paginatorRight} rowClassName="cursor-pointer">
            <Column field="branch" header="Branch" body={pTemplate1} style={{ minWidth: "8rem" }} />
            <Column field="isDefault" header="isDefault" body={checkboxTemplate2} style={{ minWidth: "8rem" }} />
