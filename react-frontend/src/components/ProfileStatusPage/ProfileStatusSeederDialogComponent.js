...
import seederData from "./profileStatus.seeder.json";
...
const ProfileStatusSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("profileStatus").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ProfileStatusSeederDialogComponent);
