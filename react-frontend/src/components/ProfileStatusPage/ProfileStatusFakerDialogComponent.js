...
import profileStatusFakerFactory from "./profileStatusFakerFactory";
...
const ProfileStatusFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = profileStatusFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("profileStatus").create(elem));
...
export default connect(mapState, mapDispatch)(ProfileStatusFakerDialogComponent);
