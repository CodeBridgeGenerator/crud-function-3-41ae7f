...
import teamsFakerFactory from "./teamsFakerFactory";
...
const TeamsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    
                const [userIdsIds, setuserIdsIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
              setuserIdsIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = teamsFakerFactory(fakerCount,userIdsIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("teams").create(elem));
...
export default connect(mapState, mapDispatch)(TeamsFakerDialogComponent);
