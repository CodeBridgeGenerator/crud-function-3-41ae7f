...
import seederData from "./ipLoginAccess.seeder.json";
...
const IpLoginAccessSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("ipLoginAccess").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(IpLoginAccessSeederDialogComponent);
