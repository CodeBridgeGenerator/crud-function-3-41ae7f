...
const IpLoginAccessDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
     
    const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.startIp}</p>
    const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.endIp}</p>
    const pTemplate4 = (rowData, { rowIndex }) => <p >{rowData.allowedStartTime}</p>
    const pTemplate5 = (rowData, { rowIndex }) => <p >{rowData.allowedEndTime}</p>
    const pTemplate6 = (rowData, { rowIndex }) => <p >{rowData.blockedStartTime}</p>
    const pTemplate7 = (rowData, { rowIndex }) => <p >{rowData.blockedEndTime}</p>
    ...
    return (
        ...
        currentPageReportTemplate="{first} to {last} of {totalRecords}" paginatorLeft={paginatorLeft} paginatorRight={paginatorRight} rowClassName="cursor-pointer">
            <Column field="startIp" header="Start Ip" body={pTemplate2} style={{ minWidth: "8rem" }} />
            <Column field="endIp" header="End Ip" body={pTemplate3} style={{ minWidth: "8rem" }} />
            <Column field="allowedStartTime" header="allowedStartTime" body={pTemplate4} style={{ minWidth: "8rem" }} />
            <Column field="allowedEndTime" header="allowedEndTime" body={pTemplate5} style={{ minWidth: "8rem" }} />
            <Column field="blockedStartTime" header="blockedStartTime" body={pTemplate6} style={{ minWidth: "8rem" }} />
            <Column field="blockedEndTime" header="blockedEndTime" body={pTemplate7} style={{ minWidth: "8rem" }} />
...
export default IpLoginAccessDataTable;