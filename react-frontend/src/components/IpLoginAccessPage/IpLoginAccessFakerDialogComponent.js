...
import ipLoginAccessFakerFactory from "./ipLoginAccessFakerFactory";
...
const IpLoginAccessFakerDialogComponent = (props) => {
...
client
    ...
    const onRun = async () => {
        let fakeData = ipLoginAccessFakerFactory(fakerCount,userIdIds,roleIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("ipLoginAccess").create(elem));
...
export default connect(mapState, mapDispatch)(IpLoginAccessFakerDialogComponent);
