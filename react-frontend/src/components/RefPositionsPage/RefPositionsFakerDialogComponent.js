...
import refPositionsFakerFactory from "./refPositionsFakerFactory";
...
const RefPositionsFakerDialogComponent = (props) => {
...
client
    ...
    const onRun = async () => {
        let fakeData = refPositionsFakerFactory(fakerCount,roleIdIds,departmentIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("refPositions").create(elem));
...
export default connect(mapState, mapDispatch)(RefPositionsFakerDialogComponent);
