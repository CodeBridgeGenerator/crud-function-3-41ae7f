...
import mobileDevicesFakerFactory from "./mobileDevicesFakerFactory";
...
const MobileDevicesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    
                const [mobilenumberidIds, setmobilenumberidIds] = useState();
                

                const [deviceidIds, setdeviceidIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
              setmobilenumberidIds(data.data);
            });
client
            .service("mobileDevices")
            .find({ query: { $select: ["_id","osff"] } })
            .then((data) => {
              setdeviceidIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = mobileDevicesFakerFactory(fakerCount,mobilenumberidIds,deviceidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("mobileDevices").create(elem));
...
export default connect(mapState, mapDispatch)(MobileDevicesFakerDialogComponent);
