...
const DepartmentsDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
     
    const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.department}</p>
    const checkboxTemplate3 = (rowData, { rowIndex }) => <Checkbox checked={rowData.isDefault}  ></Checkbox>
    ...
    return (
        ...
        currentPageReportTemplate="{first} to {last} of {totalRecords}" paginatorLeft={paginatorLeft} paginatorRight={paginatorRight} rowClassName="cursor-pointer">
            <Column field="department" header="Department" body={pTemplate2} style={{ minWidth: "8rem" }} />
            <Column field="isDefault" header="isDefault" body={checkboxTemplate3} style={{ minWidth: "8rem" }} />
