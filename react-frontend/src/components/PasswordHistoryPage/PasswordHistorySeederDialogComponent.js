...
import seederData from "./passwordHistory.seeder.json";
...
const PasswordHistorySeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("passwordHistory").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(PasswordHistorySeederDialogComponent);
