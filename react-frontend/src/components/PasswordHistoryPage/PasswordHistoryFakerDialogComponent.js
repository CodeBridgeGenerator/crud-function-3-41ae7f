...
import passwordHistoryFakerFactory from "./passwordHistoryFakerFactory";
...
const PasswordHistoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = passwordHistoryFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("passwordHistory").create(elem));
...
export default connect(mapState, mapDispatch)(PasswordHistoryFakerDialogComponent);
