...
import fieldMetaFakerFactory from "./fieldMetaFakerFactory";
...
const FieldMetaFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = fieldMetaFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("fieldMeta").create(elem));
...
export default connect(mapState, mapDispatch)(FieldMetaFakerDialogComponent);
