...
import seederData from "./fieldMeta.seeder.json";
...
const FieldMetaSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("fieldMeta").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(FieldMetaSeederDialogComponent);
