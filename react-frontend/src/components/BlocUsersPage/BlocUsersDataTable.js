...
const BlocUsersDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
     
    const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.type}</p>
    const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.pattern}</p>
    const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.detail}</p>
    ...
    return (
        ...
        currentPageReportTemplate="{first} to {last} of {totalRecords}" paginatorLeft={paginatorLeft} paginatorRight={paginatorRight} rowClassName="cursor-pointer">
            ...
            <Column field="pattern" header="Pattern" body={pTemplate1} style={{ minWidth: "8rem" }} />
            <Column field="detail" header="Detail" body={pTemplate2} style={{ minWidth: "8rem" }} />
...
export default BlocUsersDataTable;