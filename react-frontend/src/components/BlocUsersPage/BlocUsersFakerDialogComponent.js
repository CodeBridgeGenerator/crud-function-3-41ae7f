...
import blocUsersFakerFactory from "./blocUsersFakerFactory";
...
const BlocUsersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = blocUsersFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("blocUsers").create(elem));
...
export default connect(mapState, mapDispatch)(BlocUsersFakerDialogComponent);
