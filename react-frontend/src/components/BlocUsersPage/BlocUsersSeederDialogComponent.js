...
import seederData from "./blocUsers.seeder.json";
...
const BlocUsersSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("blocUsers").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(BlocUsersSeederDialogComponent);
