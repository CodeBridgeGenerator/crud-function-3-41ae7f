...
import seederData from "./fieldPermissions.seeder.json";
...
const FieldPermissionsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("fieldPermissions").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(FieldPermissionsSeederDialogComponent);
