...
import fieldPermissionsFakerFactory from "./fieldPermissionsFakerFactory";
...
const FieldPermissionsFakerDialogComponent = (props) => {
...
client
    ...
    const onRun = async () => {
        let fakeData = fieldPermissionsFakerFactory(fakerCount,roleIdIds,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("fieldPermissions").create(elem));
...
export default connect(mapState, mapDispatch)(FieldPermissionsFakerDialogComponent);
