...
const PhonesDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
     
    const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.type}</p>
    const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.number}</p>
    const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.country}</p>
    const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.phone}</p>
    ...
    return (
        ...
        currentPageReportTemplate="{first} to {last} of {totalRecords}" paginatorLeft={paginatorLeft} paginatorRight={paginatorRight} rowClassName="cursor-pointer">
            <Column field="type" header="Type" body={pTemplate0} style={{ minWidth: "8rem" }} />
            <Column field="number" header="Number" body={pTemplate1} style={{ minWidth: "8rem" }} />
            <Column field="country" header="Country" body={pTemplate2} style={{ minWidth: "8rem" }} />
            <Column field="phone" header="Phone " body={pTemplate3} style={{ minWidth: "8rem" }} />
...
export default PhonesDataTable;