...
import phonesFakerFactory from "./phonesFakerFactory";
...
const PhonesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = phonesFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("phones").create(elem));
...
export default connect(mapState, mapDispatch)(PhonesFakerDialogComponent);
