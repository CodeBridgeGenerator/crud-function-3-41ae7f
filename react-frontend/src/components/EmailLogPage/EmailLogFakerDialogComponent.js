...
import emailLogFakerFactory from "./emailLogFakerFactory";
...
const EmailLogFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    
                const [fromUserIdIds, setfromUserIdIds] = useState();
                

                const [toffIds, settoffIds] = useState();
                

                const [ccffIds, setccffIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
              setfromUserIdIds(data.data);
            });
client
            .service("users")
            .find({ query: { $select: ["_id","name"] } })
            .then((data) => {
              settoffIds(data.data);
            });
client
            .service("users")
            .find({ query: { $select: ["_id","name"] } })
            .then((data) => {
              setccffIds(data.data);
            });
client
            .service("users")
            .find({ query: { $select: ["_id","name"] } })
    ...
    const onRun = async () => {
        let fakeData = emailLogFakerFactory(fakerCount,fromUserIdIds,toffIds,ccffIds,bccIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("emailLog").create(elem));
...
export default connect(mapState, mapDispatch)(EmailLogFakerDialogComponent);
