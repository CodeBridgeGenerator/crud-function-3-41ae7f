...
import preferencesFakerFactory from "./preferencesFakerFactory";
...
const PreferencesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = preferencesFakerFactory(fakerCount,useridIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("preferences").create(elem));
...
export default connect(mapState, mapDispatch)(PreferencesFakerDialogComponent);
