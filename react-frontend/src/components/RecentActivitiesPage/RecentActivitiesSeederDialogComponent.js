...
import seederData from "./recentActivities.seeder.json";
...
const RecentActivitiesSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("recentActivities").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(RecentActivitiesSeederDialogComponent);
