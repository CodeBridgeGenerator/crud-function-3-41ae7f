...
import recentActivitiesFakerFactory from "./recentActivitiesFakerFactory";
...
const RecentActivitiesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = recentActivitiesFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("recentActivities").create(elem));
...
export default connect(mapState, mapDispatch)(RecentActivitiesFakerDialogComponent);
