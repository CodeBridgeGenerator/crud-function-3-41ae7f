...
import positionRoleDepartmentFakerFactory from "./positionRoleDepartmentFakerFactory";
...
const PositionRoleDepartmentFakerDialogComponent = (props) => {
...
client
            ...
            .find({ query: { $select: ["_id","position"] } })
            .then((data) => {
...
client
    ...
    const onRun = async () => {
        let fakeData = positionRoleDepartmentFakerFactory(fakerCount,userIdIds,positionIdIds,roleIdIds,departmentIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("positionRoleDepartment").create(elem));
...
export default connect(mapState, mapDispatch)(PositionRoleDepartmentFakerDialogComponent);
