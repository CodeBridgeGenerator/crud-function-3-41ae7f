...
import seederData from "./positionRoleDepartment.seeder.json";
...
const PositionRoleDepartmentSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("positionRoleDepartment").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(PositionRoleDepartmentSeederDialogComponent);
