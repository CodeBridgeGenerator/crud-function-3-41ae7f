...
import seederData from "./userCompanies.seeder.json";
...
const UserCompaniesSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("userCompanies").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(UserCompaniesSeederDialogComponent);
