...
import userCompaniesFakerFactory from "./userCompaniesFakerFactory";
...
const UserCompaniesFakerDialogComponent = (props) => {
    ...
    const [loading, setLoading] = useState(false);
                ...
                const [companIdIds, setcompanIdIds] = useState();
...
client
            ...
            .then((data) => {
              setcompanIdIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = userCompaniesFakerFactory(fakerCount,userIdIds,companIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userCompanies").create(elem));
...
export default connect(mapState, mapDispatch)(UserCompaniesFakerDialogComponent);
