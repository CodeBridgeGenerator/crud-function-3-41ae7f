...
import seederData from "./servicePermissions.seeder.json";
...
const ServicePermissionsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("servicePermissions").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ServicePermissionsSeederDialogComponent);
