...
import servicePermissionsFakerFactory from "./servicePermissionsFakerFactory";
...
const ServicePermissionsFakerDialogComponent = (props) => {
...
client
    ...
    const onRun = async () => {
        let fakeData = servicePermissionsFakerFactory(fakerCount,roleIdIds,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("servicePermissions").create(elem));
...
export default connect(mapState, mapDispatch)(ServicePermissionsFakerDialogComponent);
