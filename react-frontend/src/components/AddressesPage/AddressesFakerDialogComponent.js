...
import addressesFakerFactory from "./addressesFakerFactory";
...
const AddressesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    
                const [morphidIds, setmorphidIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
              setmorphidIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = addressesFakerFactory(fakerCount,morphidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("addresses").create(elem));
...
export default connect(mapState, mapDispatch)(AddressesFakerDialogComponent);
