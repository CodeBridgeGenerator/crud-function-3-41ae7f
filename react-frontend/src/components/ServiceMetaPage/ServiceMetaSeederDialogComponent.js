...
import seederData from "./serviceMeta.seeder.json";
...
const ServiceMetaSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("serviceMeta").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ServiceMetaSeederDialogComponent);
