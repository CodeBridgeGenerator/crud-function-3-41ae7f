...
import serviceMetaFakerFactory from "./serviceMetaFakerFactory";
...
const ServiceMetaFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = serviceMetaFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("serviceMeta").create(elem));
...
export default connect(mapState, mapDispatch)(ServiceMetaFakerDialogComponent);
