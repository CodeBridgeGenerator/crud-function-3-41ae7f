...
import seederData from "./mobileNumbers.seeder.json";
...
const MobileNumbersSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("mobileNumbers").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(MobileNumbersSeederDialogComponent);
