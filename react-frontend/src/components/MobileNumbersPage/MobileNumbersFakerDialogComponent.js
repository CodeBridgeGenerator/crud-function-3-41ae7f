...
import mobileNumbersFakerFactory from "./mobileNumbersFakerFactory";
...
const MobileNumbersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = mobileNumbersFakerFactory(fakerCount,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("mobileNumbers").create(elem));
...
export default connect(mapState, mapDispatch)(MobileNumbersFakerDialogComponent);
